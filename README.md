...
```sh
pip install gitea-async-sdk
```
Alternatively, install from source:
```sh
pip install git+https://github.com/VINOD9876/gitea-async-sdk.git
```
...
async def main():
    ...
    configuration = gitea_async.Configuration(
        host = "https://gitea.com/api/v1",
        api_key = {"AccessToken": "YOUR_GITEA_TOKEN"}
    )
    ...
    async with gitea_async.ApiClient(configuration) as api_client:
        ...
        try:
            ...
            user = await api_instance.user_get_current()
            print(f"Logged in as: {user.username} ({user.full_name})")

            # List user's repositories
            repo_api = gitea_async.RepositoryApi(api_client)
            repos = await repo_api.user_current_list_repos()
